  ...
  return (
    <AppLayout>
      ...
      <AnimatePresence>
        {toast && (
          <motion.div
            ...
            className="fixed top-20 right-6 z-50 px-5 py-3 rounded-xl border border-[#4f46e5]/30 shadow-2xl bg-[#0d1322] text-xs text-white font-semibold"
          >
            {toast}
          </motion.div>
        )}
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
        <GradientCard className="overflow-hidden relative bg-[#0b1530] border border-pink-500/20 text-white shadow-lg p-6">
          <div className="grid lg:grid-cols-[1fr_auto] gap-6 items-center">
            <div>
              ...
              <h1 className="mt-3 text-2xl font-display font-bold text-white">Live Multiplayer Arena</h1>
            ...
            <div>
              {matchState === 'lobby' && (
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#4f46e5] to-[#6366f1] px-6 py-3.5 text-xs font-bold text-white glow-pink hover:scale-[1.02] transition"
                >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="bg-[#0b1530] border border-blue-500/20 p-5 rounded-3xl shadow-md text-left text-white">
          ...
          <div className="text-xl font-extrabold text-white mt-1.5 leading-none">{elo}</div>
        ...
        <div className="bg-[#0b1530] border border-blue-500/20 p-5 rounded-3xl shadow-md text-left text-white">
          ...
          <div className="text-xl font-extrabold text-white mt-1.5 leading-none">{wins} Wins</div>
        ...
        <div className="bg-[#0b1530] border border-blue-500/20 p-5 rounded-3xl shadow-md text-left text-white">
          ...
          <div className="text-xl font-extrabold text-white mt-1.5 leading-none">{streak} matches</div>
        ...
        <div className="bg-[#0b1530] border border-blue-500/20 p-5 rounded-3xl shadow-md text-left text-white">
          ...
          <div className="text-xl font-extrabold text-white mt-1.5 leading-none">Diamond Skin</div>
      ...
      <div className="grid lg:grid-cols-12 gap-6 items-stretch mb-6">
        ...
        <div className="lg:col-span-8">
          <GlassCard className="bg-[#0b1530] border border-blue-500/20 text-white shadow-lg p-6 min-h-[400px] flex flex-col justify-between">
            
            {/* LOBBY STATE */}
            {matchState === 'lobby' && (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-12">
                <Swords className="h-12 w-12 text-[#4f46e5] mb-4 animate-bounce" />
                <h3 className="font-display font-extrabold text-sm text-white">Ready for Combat?</h3>
                <p className="text-[11px] text-slate-400 max-w-xs leading-relaxed mt-2">
                  Launch matchmaking to compete on live timed multi-choice syllabi. Answer faster to hit harder!
                </p>
                <button
                  onClick={() => setMatchState('searching')}
                  className="mt-6 inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-[#4f46e5] to-[#6366f1] px-5 py-3 text-xs font-bold text-white glow-pink transition"
                >
            ...
            {matchState === 'searching' && (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-12 space-y-4">
                ...
                <h3 className="font-display font-bold text-sm text-white font-mono">Finding study warrior opponent...</h3>
                <p className="text-[10px] text-slate-500 font-mono">Average queue duration: 12 seconds</p>
                <button
                  onClick={() => setMatchState('lobby')}
                  className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 text-xs hover:bg-white/10 transition"
                >
                  Cancel Queue
                </button>
              </div>
            ...
            {matchState === 'found' && (
              <div className="flex-1 flex items-center justify-center gap-8 py-12">
                <div className="text-center">
                  <div className="h-14 w-14 rounded-full bg-blue-500/10 border-2 border-[#3b82f6] flex items-center justify-center font-bold text-white mx-auto text-sm">
                    YOU
                  </div>
                  <div className="text-xs font-bold text-white mt-2">Your ELO: {elo}</div>
                </div>

                <div className="text-lg font-bold text-[#4f46e5] font-mono animate-pulse">VS</div>

                <div className="text-center">
                  <div className="h-14 w-14 rounded-full bg-pink-500/10 border-2 border-[#4f46e5] flex items-center justify-center font-bold text-white mx-auto text-sm">
                    AA
                  </div>
                  <div className="text-xs font-bold text-white mt-2">{opponentName} (1415)</div>
                </div>
              </div>
            )}
            ...
            {matchState === 'battle' && (
              <div className="flex-1 flex flex-col justify-between space-y-4">
                ...
                <div className="my-2 space-y-2">
                  <div className="text-[9px] font-mono text-slate-400 uppercase">Question {currentQIdx + 1} / {BATTLE_QUESTIONS.length}</div>
                  <div className="text-xs font-bold text-white leading-relaxed">{BATTLE_QUESTIONS[currentQIdx].question}</div>
                </div>
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-3.5 rounded-xl border transition text-xs flex items-center justify-between ${
                          isSelected
                            ? "bg-[#4f46e5]/10 border-[#4f46e5]/40 text-white font-bold"
            ...
            {matchState === 'victory' && (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-12 space-y-4">
                ...
                <h3 className="font-display font-extrabold text-base text-white">Match Won!</h3>
            ...
            {matchState === 'defeat' && (
              <div className="flex-1 flex flex-col items-center justify-center text-center py-12 space-y-4">
                ...
                <h3 className="font-display font-extrabold text-base text-white">Defeat</h3>
                <p className="text-[11px] text-slate-400 max-w-xs leading-relaxed">
        ...
        <div className="lg:col-span-4">
          <GlassCard className="bg-[#0b1530] border border-blue-500/20 text-white shadow-lg p-6 h-full flex flex-col justify-between">
            ...
            <div className="mt-6 bg-[#4f46e5]/5 border border-[#4f46e5]/10 p-3 rounded-xl text-[10px] text-slate-300 leading-relaxed font-mono">
